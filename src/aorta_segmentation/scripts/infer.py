...
class InferClass:
    def __init__(self, config_file: Optional[Union[str, Sequence[str]]] = None, **override):
        ...
        #self.model = self.model.to(torch.device("mps") if torch.backends.mps.is_available() else "cpu")
    ...
    def infer(self, image_file, save_mask=False):
        ...
        try:
            logger.debug(f"Working on {image_file} on CPU.")
            batch_data["pred"] = sliding_window_inference(
            ...
            if not any(x in str(e).lower() for x in ("memory", "cuda", "cudnn", "cpu")):
                raise e
